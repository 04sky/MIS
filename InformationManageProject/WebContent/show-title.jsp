...
<body>
		...
		<% 
        	List<Titles> departments = (List<Titles>)request.getAttribute("titles"); 
    	%>
        <div class="sidebar sidebar-hide-to-small sidebar-shrink sidebar-gestures">
            <div class="nano">
                <div class="nano-content">
                    <div class="logo"><a href="index.html"><span>Sky管理系统</span></a></div>
                    <ul>
                        <li class="label">主页</li>
                        <li><a class="sidebar-sub-toggle"><i class="ti-home"></i> 主界面 </a>
                        </li>
                        ...
                        <div class="page-header">
                            <div class="page-title">
                                <ol class="breadcrumb">
                                    ...
                                    <li class="breadcrumb-item active">职称展示</li>
                                </ol>
                            </div>
                        </div>
                    </div>
                                        ...
                                        <table class="table table-hover ">
                                            ...
                                            <tbody>
                                            	<c:forEach items="${titles}" var="title">
                                            		<tr> 
										                <td scope="row">${title.id}</td> 
										                <td>${title.name}</td> 
										                <td>${title.baseWage}</td> 
										                <td>${title.roomSupply}</td>
										                <td>${title.jobSupply}</td>
										                <c:if test="${title.status eq '1' }">
										                	<td><span class="badge badge-success">在用</span></td>
										                	<td>
										                	<a href="${pageContext.request.contextPath}/download.title?operation=download&id=${title.id}">停用</a>/
										                	<a href="${pageContext.request.contextPath}/update.title?operation=showOne&id=${title.id}">修改</a>/
										                	<a href=" ${pageContext.request.contextPath}/delete.title?operation=delete&id=${title.id}">删除</a>
										                	</td>
										                </c:if>
										                <c:if test="${title.status eq '2' }">
										                	<td><span class="badge badge-danger">停用</span></td>
										                	<td>
										                	<a href="${pageContext.request.contextPath}/upload.title?operation=upload&id=${title.id}">启用</a>/
										                	<a href="${pageContext.request.contextPath}/showOne.title?operation=showOne&id=${title.id}">修改</a>/
										                	<a href="${pageContext.request.contextPath}/delete.title?operation=delete&id=${title.id}">删除</a>
										                	</td>
										                </c:if>
										            </tr>
